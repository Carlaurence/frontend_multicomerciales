...
const EngineManufacturerslist = () => {
     ...
     * 3- SI ESCOGEMOS [SI], SE EJECUTA EL REQUEST deteleteEngineManufacturer(id) Y SE ELIMINARA **********
     ****************************************************************************************************/
    const deteleteEngineManufacturer = async (id) => {//SE INVOCA DESDE warningDelete() PORQUE 'swal' NO ACEPTA ASYNC/AWAIT
        const response = await crud.DELETE(`/api/enginemanufacturer/${id}`);
    ...
    const warningDelete = (id) => {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    deteleteEngineManufacturer(id);//SE DEBE INVOCAR PORQUE NO 'swal' NO ACEPTA UN ASYNC / AWAIT
                } else {
    ...
    /*TERCERA PARTE: **************************************************************************************
     * 1- AL CLICKEAR EL ICON [EDIT], VIAJAMOS A LA URL /update_enginemanufacturer/${manufacturer._id} **********
     ****************************************************************************************************/
                ...
                <div className="flex justify-center w-screen">
                    ...
                    <div className="relative mt-32 mb-14 bg-white border border-black shadow-2xl shadow-red-600 w-[484px] rounded-xl flex justify-center items-center">
                        <div className="bg-red-600 mt-3 mb-3 border border-white shadow-2xl shadow-black w-[460px] rounded-xl ">
                            <span className="mb-6 flex justify-center text-xl text-white pt-4 font-semibold uppercase">Lista Fabricantes de Motores</span>

                            {engineManufacturers.map((manufacturer, index) => (
                                    ...
                                    <form className="flex flex-col justify-center items-center mb-4">
                                        <div className="flex flex-row gap-4">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <BiEdit className="hover:opacity-50" onClick={() => { navigate(`/update_enginemanufacturer/${manufacturer._id}`) }} style={{ color: 'white', fontSize: '24px', cursor: "pointer" }} />
...
export default EngineManufacturerslist;